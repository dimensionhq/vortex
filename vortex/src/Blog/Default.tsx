  ...
  const components: Partial<Omit<NormalComponents, keyof SpecialComponents> & SpecialComponents> = {
    ...
    a: ({ children, ...rest }) => {
      if (rest.href?.startsWith('https://twitter.com')) {
        let id = rest.href?.match(/twitter.com\/.*\/([0-9]+).*/) ?? '1468899596730441730';
        return <TweetEmbed id={id[1]} options={{ theme: props.theme?.twitter?.theme ?? 'dark' }} />;
        ...
        let id = rest.href?.match(/codepen.io\/([(\w)+]*.)\/pen\/([(\w)+]*.)/) ?? [
          '',
          'alvaromontoro',
          'vYexLGV'
        ];
        ...
        return (
          <a href={rest.href} style={{ textDecorationColor: props.theme?.linkColor }}>
            {children}
          </a>
        );
      }
    ...
    img: (props) => <img {...props} style={{ maxWidth: '100%' }}></img>
  };
  return (
      ...
      <Main {...props} theme={props.theme}>
        ...
        <div style={{ fontFamily: props.font?.title }}>
          {props.header?.category ? (
            <Category theme={props.theme} href={props.header.category.url}>
              {props.header.category.title}
            </Category>
          ) : (
            <></>
          )}
          <Title style={{ marginTop: '20px' }} theme={props.theme}>
            {props.header?.title}
          </Title>
        </div>
